    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-6 sm:p-8 rounded-lg shadow-sm">
                ...
                {{-- Filter & Search Bar --}}
                <div class="mb-4">
                    <form action="{{ route('user-management.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                        {{-- Search Input --}}
                        ...
                        <div>
                            ...
                            <select name="branch_filter" id="branch_filter" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                <option value="">All Branches</option>
                                @foreach ($branches as $branch)
                                    <option value="{{ $branch }}" @if(request('branch_filter') == $branch) selected @endif>{{ $branch }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($users as $user)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $user->nip ?? 'N/A' }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $user->email }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $user->role }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $user->branch ?? 'N/A' }}</td>
